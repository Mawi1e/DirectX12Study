...
namespace Mawi1e {
	...
	void D3DApp::BuildRenderItems_New() {
		...
		boxRitems->Mat = m_Materials["testPNGTex"].get();
		...
		auto boxRitems2 = std::make_unique<RenderItem>();
		XMStoreFloat4x4(&boxRitems2->World, XMMatrixTranslation(2.0f, 0.0f, 0.0f));
		boxRitems2->Geo = m_DrawArgs["boxGeo"].get();
		boxRitems2->Mat = m_Materials["boxTex"].get();
		boxRitems2->BaseVertexLocation = boxRitems2->Geo->DrawArgs["box"].BaseVertexLocation;
		boxRitems2->StartIndexLocation = boxRitems2->Geo->DrawArgs["box"].StartIndexLocation;
		boxRitems2->IndexCount = boxRitems2->Geo->DrawArgs["box"].IndexCount;
		boxRitems2->ObjCBIndex = 1;
		boxRitems2->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

		m_AllRItems.push_back(std::move(boxRitems2));
	...
	void D3DApp::BuildDescriptorHeaps() {
		...
		auto woodCrateTex = m_Textures["woodTexCoord"]->Resource;
		...
		auto testPngTex = m_Textures["TestPNG"]->Resource;
		srvDesc.Format = testPngTex->GetDesc().Format;
		srvDesc.Texture2D.MipLevels = testPngTex->GetDesc().MipLevels;
		m_Device->CreateShaderResourceView(testPngTex.Get(), &srvDesc, hDescriptor);
	}
	...
	void D3DApp::BuildMaterials() {
		...
		auto testPNGTex = std::make_unique<Material>();
		testPNGTex->Name = "testPNGTex";
		testPNGTex->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
		testPNGTex->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
		testPNGTex->Roughness = 0.2f;
		testPNGTex->MatCBIndex = 1;
		testPNGTex->DiffuseSrvHeapIndex = 1;

		m_Materials["testPNGTex"] = std::move(testPNGTex);
	}
	...
	void D3DApp::LoadTextures() {
		...
		tex->name = "woodTexCoord";
		tex->fileName = L"WoodCrate01.dds";

		THROWFAILEDIF("@@@ Error: DirectX::CreateDDSTextureFromFile12", DirectX::CreateDDSTextureFromFile12(m_Device.Get(),
			m_CommandList.Get(), tex->fileName.c_str(), tex->Resource, tex->Uploader));
		m_Textures[tex->name] = std::move(tex);


		auto texPng = std::make_unique<Texture>();
		texPng->name = "TestPNG";
		texPng->fileName = L"test.jpg";

		CreateWICTextureFromFile12(m_Device.Get(),
			m_CommandList.Get(), texPng->fileName.c_str(), texPng->Resource, texPng->Uploader);
		m_Textures[texPng->name] = std::move(texPng);
	}
