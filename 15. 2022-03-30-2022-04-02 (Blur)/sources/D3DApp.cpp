...
namespace Mawi1e {
	...
	void D3DApp::Initialize(const D3DSettings& d3dSettings) {
		...
		m_BlurFillter = std::make_unique<BlurFilter>(m_Device.Get(),
			m_d3dSettings.screenWidth, m_d3dSettings.screenHeight, m_BackBufferFormat);
	...
	void D3DApp::Draw(const GameTimer* gameTimer) {
		...
		m_BlurFillter->Execute(m_CommandList.Get(), m_PostRootSignature.Get(),
			m_PSOs["CSHori"].Get(), m_PSOs["CSVert"].Get(), m_RtvDescriptor[m_CurrBackBufferIdx].Get(), 4);

		m_CommandList->ResourceBarrier(1, &My_unmove(CD3DX12_RESOURCE_BARRIER::Transition(m_RtvDescriptor[m_CurrBackBufferIdx].Get(),
			D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST)));

		m_CommandList->CopyResource(m_RtvDescriptor[m_CurrBackBufferIdx].Get(), m_BlurFillter->Output());
	...
	void D3DApp::ResizeBuffer() {
		...
		if (m_BlurFillter != nullptr) {
			m_BlurFillter->ResizeBuffer(m_d3dSettings.screenWidth, m_d3dSettings.screenHeight);
		}
	}
	...
	void D3DApp::BuildPostRootSignature() {
		...
		cbvParameter[0].InitAsConstants(12, 0);
		cbvParameter[1].InitAsDescriptorTable(1, &srvRange);
		cbvParameter[2].InitAsDescriptorTable(1, &uavRange);
		...
		hResult = m_Device->CreateRootSignature(0, serialRootSig->GetBufferPointer(),
			serialRootSig->GetBufferSize(), IID_PPV_ARGS(m_PostRootSignature.GetAddressOf()));
	...
	void D3DApp::BuildShadersAndInputlayout() {
		...
		m_Shaders["HoriCS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_CS, nullptr, "HoriCS", "cs_5_0");
		m_Shaders["VertCS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_CS, &alphatest[0], "VertCS", "cs_5_0");
	...
	void D3DApp::BuildPSO() {
		...
		D3D12_COMPUTE_PIPELINE_STATE_DESC ComputeVertsPSO = {};
		ComputeVertsPSO.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
		ComputeVertsPSO.NodeMask = 0;
		ComputeVertsPSO.pRootSignature = m_PostRootSignature.Get();
		ComputeVertsPSO.CS = {
			reinterpret_cast<BYTE*>(m_Shaders["HoriCS"]->GetBufferPointer()),
			m_Shaders["HoriCS"]->GetBufferSize()
		};
		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateComputePipelineState(&ComputeVertsPSO, IID_PPV_ARGS(m_PSOs["CSHori"].GetAddressOf())));


		ComputeVertsPSO.CS = {
			reinterpret_cast<BYTE*>(m_Shaders["VertCS"]->GetBufferPointer()),
			m_Shaders["VertCS"]->GetBufferSize()
		};


		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateComputePipelineState(&ComputeVertsPSO, IID_PPV_ARGS(m_PSOs["CSVert"].GetAddressOf())));
	}
	...
	void D3DApp::BuildDescriptorHeaps() {
		...
		srvHeapDesc.NumDescriptors = 4 + 4;
		...
		m_BlurFillter->BuildDescriptor(
			CD3DX12_CPU_DESCRIPTOR_HANDLE(m_SrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 4, m_CbvSize),
			CD3DX12_GPU_DESCRIPTOR_HANDLE(m_SrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 4, m_CbvSize),
			m_CbvSize);
	}
