...
namespace Mawi1e {
	...
	void D3DApp::Draw(const GameTimer* gameTimer) {
		...
		if (m_IsWireFrames) { m_CommandList->SetPipelineState(m_PSOs["ice_wireframe"].Get()); }
		else { m_CommandList->SetPipelineState(m_PSOs["ice_opaque"].Get()); }
		DrawRenderItems(m_CommandList.Get(), mRitemLayer[(int)RenderLayer::OpaqueGS]);
	...
	void D3DApp::BuildPSO() {
		...
		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateGraphicsPipelineState(&TreeSpritePSODesc, IID_PPV_ARGS(m_PSOs["ice_opaque"].GetAddressOf())));

		D3D12_GRAPHICS_PIPELINE_STATE_DESC iceWirePSODesc = TreeSpritePSODesc;
		iceWirePSODesc.RasterizerState.FillMode = D3D12_FILL_MODE_WIREFRAME;

		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateGraphicsPipelineState(&iceWirePSODesc, IID_PPV_ARGS(m_PSOs["ice_wireframe"].GetAddressOf())));
	}
	...
	void D3DApp::UpdateObjectCBs() {
		...
		for (auto& e : m_AllRItems) {
			if (e->NumFramesDirty > 0) {
				...
				XMStoreFloat4x4(&objConstants.WorldInvTranspose, XMMatrixTranspose(XMMatrixTranspose(invWorld)));
	...
	void D3DApp::LoadTexture() {
		...
		treeArrayTex->filename = L"./ice.dds";
		THROWFAILEDIF("@@@ Error: DirectX::CreateDDSTextureFromFile12", DirectX::CreateDDSTextureFromFile12(m_Device.Get(), m_CommandList.Get(),
	...
	void D3DApp::BuildRenderItems_New() {
		...
		XMStoreFloat4x4(&treeSpriteRitems->World, XMMatrixTranslation(0.0f, 20.0f, 0.0f));
		treeSpriteRitems->Geo = m_DrawArgs["geoSphereGeo"].get();
		treeSpriteRitems->Mat = m_Materials["treeSprite"].get();
		treeSpriteRitems->BaseVertexLocation = treeSpriteRitems->Geo->DrawArgs["geosphere"].BaseVertexLocation;
		treeSpriteRitems->StartIndexLocation = treeSpriteRitems->Geo->DrawArgs["geosphere"].StartIndexLocation;
		treeSpriteRitems->IndexCount = treeSpriteRitems->Geo->DrawArgs["geosphere"].IndexCount;
		treeSpriteRitems->ObjCBIndex = 3;
		treeSpriteRitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

		mRitemLayer[(int)RenderLayer::OpaqueGS].push_back(treeSpriteRitems.get());
	...
	void D3DApp::BuildDescriptorHeaps() {
		...
		auto iceTex = m_Textures["treeArrayTex"]->GPUResource;
	...
	void D3DApp::BuildTreeSprites() {
		GeometryGenerator geoGen;
		auto geoSphere = geoGen.CreateGeosphere(10.0f, 2);
		//auto geoSphere = geoGen.CreateBox(8.0f, 8.0f, 8.0f, 3);
		...
		MeshBox->DrawArgs["geosphere"] = subMeshGeo;
