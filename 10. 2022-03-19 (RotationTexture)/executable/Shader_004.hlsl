...
SamplerState gsamState : register(s0);
...
float2 rotation_uv(float2 uv, float degree) {
    const float rad1 = (3.14159265359f * 1.0f) / 180.0f;
    float rotationRadian = degree * rad1;

    float s = sin(rotationRadian);
    float c = cos(rotationRadian);
...
{
    float2 uv_rot = rotation_uv(pin.TexC, gTimeDeltaAcc);
    float4 diffuseAlbedo1 = gTextures[0].Sample(gsamState, uv_rot) * gDiffuseAlbedo;
    float4 diffuseAlbedo2 = gTextures[1].Sample(gsamState, pin.TexC) * gDiffuseAlbedo;
    float4 diffuseAlbedo = diffuseAlbedo1 * diffuseAlbedo2 * 2.0f;
