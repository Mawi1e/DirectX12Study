...
namespace Mawi1e {
	...
	void D3DApp::Draw(const GameTimer* gameTimer) {
		...
		// reflection
		m_CommandList->SetGraphicsRootConstantBufferView(1, p->GetGPUVirtualAddress() + 1 * passCBByteSize);
	...
	void D3DApp::BuildRootSignature() {
		...
		cbvParameter[2].InitAsShaderResourceView(0, 1);
		cbvParameter[3].InitAsDescriptorTable(1, &dRange, D3D12_SHADER_VISIBILITY_PIXEL);
	...
	void D3DApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& rItem) {
		...
		for (size_t i = 0; i < rItem.size(); ++i) {
			...
			D3D12_GPU_VIRTUAL_ADDRESS objCBAddress = currObjCB->GetGPUVirtualAddress() + ri->ObjCBIndex * objCBByteSize;
