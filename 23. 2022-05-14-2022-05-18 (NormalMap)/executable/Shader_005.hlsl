...
Texture2D gTextures[9] : register(t1);
...
float4 PS(VertexOut pin) : SV_Target
{
    ...
    int normalMapIndex = matBuffer.NormalSrvHeapIndex;
    ...
    float4 normalSample = 0.0f;
    float3 bumpedNormalW = 0.0f;
    ...
    if (normalMapIndex != -1) {
        normalSample = gTextures[normalMapIndex].Sample(gsamLinearWrap, pin.TexC);
        bumpedNormalW = NormalSampleToWorldSpace(normalSample.rgb, pin.TangentW, pin.NormalW);
        shininess *= normalSample.a;
    }
    ...
    float4 refle_color = gCubeMap.Sample(gsamLinearWrap, r);
    float3 schlickFactor = SchlickFresnel(FresnelR0, bumpedNormalW, r);
    litColor.rgb += shininess * schlickFactor * refle_color.rgb;
