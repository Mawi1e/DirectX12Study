...
Texture2D gTextures[7] : register(t0);

StructuredBuffer<MaterialBuffer> gMaterialBuffer : register(t0, space1);
StructuredBuffer<InstanceBuffer> gInstanceBuffer : register(t1, space1);
...
VertexOut VS(VertexIn vin, uint instIdx : SV_InstanceID)
{
	VertexOut vout = (VertexOut)0.0f;

    InstanceBuffer insBuffer = gInstanceBuffer[instIdx];
    float4x4 world = insBuffer.World;
    float4x4 texTransform = insBuffer.TexTransform;
    uint matIndex = insBuffer.MaterialIndex;
    vout.MatIndex = matIndex;


    MaterialBuffer matBuffer = gMaterialBuffer[matIndex];
    ...
    float4 texTrans = mul(float4(vin.TexC, 0.0f, 1.0f), texTransform);
    vout.TexC = mul(texTrans, matBuffer.MatTransform).xy;
...
{
    MaterialBuffer matBuffer = gMaterialBuffer[pin.MatIndex];
    float4 diffuseAlbedo = matBuffer.DiffuseAlbedo;
    float3 FresnelR0 = matBuffer.FresnelR0;
    float Roughness = matBuffer.Roughness;
    uint diffuseMapIndex = matBuffer.DiffuseMapIndex;
