...
namespace Mawi1e {
	...
	void D3DApp::Draw(const GameTimer* gameTimer) {
		...
		m_CommandList->SetPipelineState(m_PSOs["tessPSO"].Get());
		DrawRenderItems(m_CommandList.Get(), mRitemLayer[(int)RenderLayer::HSOpaque]);
	...
	void D3DApp::BuildShadersAndInputlayout() {
		...
		m_Shaders["tessVS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_TESS_VS, nullptr, "VS", "vs_5_0");
		m_Shaders["tessHS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_TESS_HS, nullptr, "HS", "hs_5_0");
		m_Shaders["tessDS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_TESS_DS, nullptr, "DS", "ds_5_0");
		m_Shaders["tessPS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_TESS_PS, nullptr, "PS", "ps_5_0");
	...
	void D3DApp::BuildPSO() {
		...
		tessPSODesc.DSVFormat = m_DepthStencilFormat;
		tessPSODesc.Flags = D3D12_PIPELINE_STATE_FLAG_NONE;
		tessPSODesc.HS = {
		...
		tessPSODesc.InputLayout = {
			m_TesselationInputElement.data(),
			m_TesselationInputElement.size(),
		};
		tessPSODesc.NodeMask = 0;
		...
		tessPSODesc.RTVFormats[0] = m_BackBufferFormat;
		tessPSODesc.SampleMask = UINT_MAX;
		tessPSODesc.SampleDesc.Count = m_MSQualityState ? 4 : 1;
		tessPSODesc.SampleDesc.Quality = m_MSQualityState ? (m_MultisampleQuality - 1) : 0;
		...
		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateGraphicsPipelineState(&tessPSODesc, IID_PPV_ARGS(m_PSOs["tessPSO"].GetAddressOf())));
	...
	void D3DApp::BuildRenderItems_New() {
		...
		auto tessGridRitems = std::make_unique<RenderItem>();
		XMStoreFloat4x4(&tessGridRitems->World, XMMatrixTranslation(10.0f, 30.0f, -10.0f));
		tessGridRitems->TexTransform = VertexBuffer::GetMatrixIdentity4x4();
		tessGridRitems->Geo = m_DrawArgs["quadpatchGeo"].get();
		tessGridRitems->Mat = m_Materials["tessWhite1x1"].get();
		tessGridRitems->BaseVertexLocation = tessGridRitems->Geo->DrawArgs["quadpatch"].BaseVertexLocation;
		tessGridRitems->StartIndexLocation = tessGridRitems->Geo->DrawArgs["quadpatch"].StartIndexLocation;
		tessGridRitems->IndexCount = tessGridRitems->Geo->DrawArgs["quadpatch"].IndexCount;
		tessGridRitems->ObjCBIndex = 4;
		tessGridRitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_4_CONTROL_POINT_PATCHLIST;

		mRitemLayer[(int)RenderLayer::HSOpaque].push_back(tessGridRitems.get());
		...
		m_AllRItems.push_back(std::move(tessGridRitems));
	}
	...
	void D3DApp::BuildDescriptorHeaps() {
		...
		D3D12_SHADER_RESOURCE_VIEW_DESC tempSrvDesc = srvDesc;
		...
		auto white1x1Tex = m_Textures["white1x1Tex"]->GPUResource;
		tempSrvDesc.Format = white1x1Tex->GetDesc().Format;
		m_Device->CreateShaderResourceView(white1x1Tex.Get(), &tempSrvDesc, srvHandle);
	}
	...
	void D3DApp::BuildMaterials() {
		...
		auto tessWhite1x1 = std::make_unique<Material>();
		tessWhite1x1->Name = "tessWhite1x1";
		tessWhite1x1->MatCBIndex = 4;
		tessWhite1x1->DiffuseSrvHeapIndex = 4;
		tessWhite1x1->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
		tessWhite1x1->FresnelR0 = XMFLOAT3(0.1f, 0.1f, 0.1f);
		tessWhite1x1->Roughness = 0.5f;

		m_Materials["grass"] = std::move(grass);
		m_Materials["water"] = std::move(water);
		m_Materials["wirefence"] = std::move(wirefence);
		m_Materials["treeSprite"] = std::move(treeSprite);
		m_Materials["tessWhite1x1"] = std::move(tessWhite1x1);
	}
	...
	void D3DApp::BuildQuadPathGridGeometry() {
	...
	void D3DApp::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& rItem) {
		...
		for (size_t i = 0; i < rItem.size(); ++i) {
			...
			D3D12_GPU_VIRTUAL_ADDRESS cbAddress1 = currObjCB->GetGPUVirtualAddress() + ri->ObjCBIndex * objCBByteSize;
			D3D12_GPU_VIRTUAL_ADDRESS cbAddress2 = currMatCB->GetGPUVirtualAddress() + ri->Mat->MatCBIndex * matCBByteSize;
