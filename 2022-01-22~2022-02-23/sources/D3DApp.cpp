...
namespace Mawi1e {
	...
	void D3DApp::BuildRenderItems() {
		...
		XMStoreFloat4x4(&boxRitems->World, XMMatrixScaling(2.0f, 2.0f, 2.0f) * XMMatrixTranslation(0.0f, 0.5f, 0.0f));
		boxRitems->Geo = m_DrawArgs["meshGeo"].get();
		...
		auto gridRitems = std::make_unique<RenderItem>();
		gridRitems->World = VertexBuffer::GetMatrixIdentity4x4();
		gridRitems->Geo = m_DrawArgs["meshGeo"].get();
		gridRitems->BaseVertexLocation = gridRitems->Geo->DrawArgs["grid"].BaseVertexLocation;
		gridRitems->StartIndexLocation = gridRitems->Geo->DrawArgs["grid"].StartIndexLocation;
		gridRitems->IndexCount = gridRitems->Geo->DrawArgs["grid"].IndexCount;
		gridRitems->ObjCBIndex = 1;
		gridRitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
		m_AllRItems.push_back(std::move(gridRitems));

		UINT ObjCBIndex = 2;
		for (int i = 0; i < 5; ++i) {
			auto LeftCylitems = std::make_unique<RenderItem>();
			auto RightCylitems = std::make_unique<RenderItem>();
			auto LeftSphitems = std::make_unique<RenderItem>();
			auto RightSphitems = std::make_unique<RenderItem>();


			auto leftCylWorld = XMMatrixTranslation(-5.0f, 1.5f, -10.0f + i * 5.0f);
			auto rightCylWorld = XMMatrixTranslation(+5.0f, 1.5f, -10.0f + i * 5.0f);

			auto leftSphWorld = XMMatrixTranslation(-5.0f, 3.5f, -10.0f + i * 5.0f);
			auto rightSphWorld = XMMatrixTranslation(+5.0f, 3.5f, -10.0f + i * 5.0f);


			XMStoreFloat4x4(&LeftCylitems->World, leftCylWorld);
			LeftCylitems->Geo = m_DrawArgs["meshGeo"].get();
			LeftCylitems->BaseVertexLocation = LeftCylitems->Geo->DrawArgs["cylinder"].BaseVertexLocation;
			LeftCylitems->StartIndexLocation = LeftCylitems->Geo->DrawArgs["cylinder"].StartIndexLocation;
			LeftCylitems->IndexCount = LeftCylitems->Geo->DrawArgs["cylinder"].IndexCount;
			LeftCylitems->ObjCBIndex = ObjCBIndex++;
			LeftCylitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

			XMStoreFloat4x4(&RightCylitems->World, rightCylWorld);
			RightCylitems->Geo = m_DrawArgs["meshGeo"].get();
			RightCylitems->BaseVertexLocation = RightCylitems->Geo->DrawArgs["cylinder"].BaseVertexLocation;
			RightCylitems->StartIndexLocation = RightCylitems->Geo->DrawArgs["cylinder"].StartIndexLocation;
			RightCylitems->IndexCount = RightCylitems->Geo->DrawArgs["cylinder"].IndexCount;
			RightCylitems->ObjCBIndex = ObjCBIndex++;
			RightCylitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

			XMStoreFloat4x4(&LeftSphitems->World, leftSphWorld);
			LeftSphitems->Geo = m_DrawArgs["meshGeo"].get();
			LeftSphitems->BaseVertexLocation = LeftSphitems->Geo->DrawArgs["sphere"].BaseVertexLocation;
			LeftSphitems->StartIndexLocation = LeftSphitems->Geo->DrawArgs["sphere"].StartIndexLocation;
			LeftSphitems->IndexCount = LeftSphitems->Geo->DrawArgs["sphere"].IndexCount;
			LeftSphitems->ObjCBIndex = ObjCBIndex++;
			LeftSphitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

			XMStoreFloat4x4(&RightSphitems->World, rightSphWorld);
			RightSphitems->Geo = m_DrawArgs["meshGeo"].get();
			RightSphitems->BaseVertexLocation = RightSphitems->Geo->DrawArgs["sphere"].BaseVertexLocation;
			RightSphitems->StartIndexLocation = RightSphitems->Geo->DrawArgs["sphere"].StartIndexLocation;
			RightSphitems->IndexCount = RightSphitems->Geo->DrawArgs["sphere"].IndexCount;
			RightSphitems->ObjCBIndex = ObjCBIndex++;
			RightSphitems->PrimitiveType = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;

			m_AllRItems.push_back(std::move(LeftCylitems));
			m_AllRItems.push_back(std::move(RightCylitems));
			m_AllRItems.push_back(std::move(LeftSphitems));
			m_AllRItems.push_back(std::move(RightSphitems));
		}
