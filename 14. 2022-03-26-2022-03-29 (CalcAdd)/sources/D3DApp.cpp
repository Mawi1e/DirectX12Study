...
namespace Mawi1e {
	...
	void D3DApp::BuildShadersAndInputlayout() {
		...
		m_Shaders["vecAddCS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_CS, nullptr, "CS", "cs_5_0");
	...
	void D3DApp::BuildPSO() {
		...
		computePsoDesc.pRootSignature = m_CalcRootSignature.Get();
		computePsoDesc.CS = {
		...
		THROWFAILEDIF("@@@Error: ID3D12Device::CreateComputePipelineState",
			m_Device->CreateComputePipelineState(&computePsoDesc, IID_PPV_ARGS(m_PSOs["vecAddPSO"].GetAddressOf())));
	}

	void D3DApp::BuildBuffers() {
		std::vector<InputA> outputABuffer;
		std::vector<InputA> outputBBuffer;

		for (size_t i = 0; i < m_NumBufferElementCount; ++i) {
			InputA A;
			A.a = DirectX::XMFLOAT3(0.0f, (float)i, 0.0f);
			A.b = DirectX::XMFLOAT2((float)i, 0.0f);

			InputA B;
			B.a = DirectX::XMFLOAT3(0.0f, (float)i, 0.0f);
			B.b = DirectX::XMFLOAT2(0.0f, (float)i);

			outputABuffer.push_back(A);
			outputBBuffer.push_back(B);
		}

		const UINT64 sizeOfA = sizeof(InputA) * outputABuffer.size();
		const UINT64 sizeOfB = sizeof(InputA) * outputBBuffer.size();

		m_InputA_GPU = VertexBuffer::CreateDefaultBuffer(m_Device.Get(), m_CommandList.Get(),
			outputABuffer.data(), sizeOfA, m_InputA_Uploader);
		m_InputB_GPU = VertexBuffer::CreateDefaultBuffer(m_Device.Get(), m_CommandList.Get(),
			outputBBuffer.data(), sizeOfB, m_InputB_Uploader);

		m_Device->CreateCommittedResource(&My_unmove(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_READBACK)),
			D3D12_HEAP_FLAG_NONE, &My_unmove(CD3DX12_RESOURCE_DESC::Buffer(sizeOfA)), D3D12_RESOURCE_STATE_COPY_DEST,
			nullptr, IID_PPV_ARGS(m_ReadBackBuffer.GetAddressOf()));

		m_Device->CreateCommittedResource(&My_unmove(CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT)),
			D3D12_HEAP_FLAG_NONE, &My_unmove(CD3DX12_RESOURCE_DESC::Buffer(sizeOfB, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS)),
			D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, IID_PPV_ARGS(m_OutputBuffer.GetAddressOf()));
	}
	...
	void D3DApp::DoComputeWork() {
		...
		m_CommandList->Reset(m_CommandAllocator.Get(), m_PSOs["vecAddPSO"].Get());

		m_CommandList->SetComputeRootSignature(m_CalcRootSignature.Get());

		m_CommandList->SetComputeRootShaderResourceView(0, m_InputA_GPU->GetGPUVirtualAddress());
		m_CommandList->SetComputeRootShaderResourceView(1, m_InputB_GPU->GetGPUVirtualAddress());
		m_CommandList->SetComputeRootUnorderedAccessView(2, m_OutputBuffer->GetGPUVirtualAddress());

		m_CommandList->Dispatch(1, 1, 1);

		m_CommandList->ResourceBarrier(1, &My_unmove(CD3DX12_RESOURCE_BARRIER::Transition(m_OutputBuffer.Get(),
		...
		InputA* output = nullptr;
		std::ofstream fout("results.txt");
		m_ReadBackBuffer->Map(0, nullptr, reinterpret_cast<void**>(&output));

		for (size_t i = 0; i < m_NumBufferElementCount; ++i) {
			fout << i << ".a: (" << output[i].a.x << ',' << output[i].a.y << ',' << output[i].a.z << ")\n";
			fout << i << ".b: (" << output[i].b.x << ',' << output[i].b.y << ")\n\n";
		}
	...
	void D3DApp::BuildRootSigCalc() {
		CD3DX12_ROOT_PARAMETER cbvParameter[3];
		cbvParameter[0].InitAsShaderResourceView(0);
		cbvParameter[1].InitAsShaderResourceView(1);
		cbvParameter[2].InitAsUnorderedAccessView(0);
		...
		THROWFAILEDIF("@@@ Error: D3D12SerializeRootSignature -> Calc", hResult);

		hResult = m_Device->CreateRootSignature(0, serialRootSig->GetBufferPointer(),
			serialRootSig->GetBufferSize(), IID_PPV_ARGS(m_CalcRootSignature.GetAddressOf()));
