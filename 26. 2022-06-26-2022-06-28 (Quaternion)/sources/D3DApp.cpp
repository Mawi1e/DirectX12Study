...
namespace Mawi1e {
	...
	D3DApp::D3DApp() {
		...
		const float radX = 20.0f;
		const float radY = 30.0f;

		m_SceneBounds.Center = { 41.0f, -43.0f, 42.0f };
		m_SceneBounds.Radius = sqrtf(radX * radX + radY * radY);
	}
	...
	void D3DApp::Initialize(const D3DSettings& d3dSettings) {
		...
		m_ShadowMap = std::make_unique<ShadowMap>(m_Device.Get(),
			gShadowMapSize, gShadowMapSize, m_DepthStencilFormat);

		m_QuatManager = std::make_unique<QuaternionManager>();
		m_QuatManager->Initailize();
	...
	void D3DApp::Update(const GameTimer* gameTimer) {
		...
		m_QuatManager->Update(gameTimer->DeltaTime(), m_SkullRitem->World);
		...
		mLightRotationAngle += 0.6f * gameTimer->DeltaTime();
		XMMATRIX R = XMMatrixRotationY(mLightRotationAngle);

		for (int i = 0; i < 3; ++i) {
			XMVECTOR LightDir = XMLoadFloat3(&mBaseLightDirections[i]);
			XMVECTOR RotLightDir = XMVector3TransformNormal(LightDir, R);
			XMStoreFloat3(&mRotatedLightDirections[i], RotLightDir);
		}

		UpdateObjectCB(gameTimer);
		UpdateMatetialCBs(gameTimer);
		UpdateShadowTransform(gameTimer);
		UpdatePassCB();
		UpdateShadowPassCB();
	}
	...
	void D3DApp::CreateDescriptorHeap() {
		...
		dsvHeapDesc.NumDescriptors = 2 + 1;
	...
	void D3DApp::BuildRootSignature() {
		...
		cubeMapRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0, 0);

		CD3DX12_DESCRIPTOR_RANGE dRange;
		// 9 - tex
		// 1 - sky
		// 3 - shadowMap
		dRange.Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 8, 2, 0);

		const size_t size = 5;

		CD3DX12_ROOT_PARAMETER cbvParameter[size];
		...
		CD3DX12_ROOT_SIGNATURE_DESC rootSignatureDesc(size, cbvParameter, (UINT)sSamplers.size(), sSamplers.data(),
	...
	void D3DApp::BuildShadersAndInputlayout() {
		...
		m_Shaders["shadowVS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_SHADOWMAP_VS, nullptr, "VS", "vs_5_1");
		m_Shaders["shadowPS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_SHADOWMAP_PS, nullptr, "PS", "ps_5_1");
		m_Shaders["shadowAlphaTestedPS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_SHADOWMAP_PS, &alphatest[0], "PS", "ps_5_1");

		m_Shaders["shadowDebugVS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_SHADOWMAP_DEBUG_VS, nullptr, "VS", "vs_5_1");
		m_Shaders["shadowDebugPS"] = VertexBuffer::CompileShader(SOURCE_SHADER_FILE_SHADOWMAP_DEBUG_PS, nullptr, "PS", "ps_5_1");
	...
	void D3DApp::BuildPSO() {
		...
		D3D12_GRAPHICS_PIPELINE_STATE_DESC ShdowMapPSODesc = GrphicsPSODesc;
		ShdowMapPSODesc.RTVFormats[0] = DXGI_FORMAT_UNKNOWN;
		ShdowMapPSODesc.NumRenderTargets = 0;
		ShdowMapPSODesc.RasterizerState.DepthBias = 10000;
		ShdowMapPSODesc.RasterizerState.DepthBiasClamp = 0;
		ShdowMapPSODesc.RasterizerState.SlopeScaledDepthBias = 1.0f;
		ShdowMapPSODesc.VS = {
		...
		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateGraphicsPipelineState(&ShdowMapPSODesc, IID_PPV_ARGS(m_PSOs["shadow_opaque"].GetAddressOf())));


		D3D12_GRAPHICS_PIPELINE_STATE_DESC ShdowMapDebugPSODesc = GrphicsPSODesc;
		ShdowMapDebugPSODesc.VS = {
			reinterpret_cast<BYTE*>(m_Shaders["shadowDebugVS"]->GetBufferPointer()),
			m_Shaders["shadowDebugVS"]->GetBufferSize(),
		};
		ShdowMapDebugPSODesc.PS = {
			reinterpret_cast<BYTE*>(m_Shaders["shadowDebugPS"]->GetBufferPointer()),
			m_Shaders["shadowDebugPS"]->GetBufferSize(),
		};

		THROWFAILEDIF("@@@Error: ID3D12Device::CreateGraphicsPipelineState",
			m_Device->CreateGraphicsPipelineState(&ShdowMapDebugPSODesc, IID_PPV_ARGS(m_PSOs["debug"].GetAddressOf())));
	}
	...
	void D3DApp::UpdateShadowTransform(const GameTimer* gameTimer) {
		XMVECTOR lightDir = XMLoadFloat3(&mRotatedLightDirections[0]);
		XMVECTOR lightPos = lightDir * m_SceneBounds.Radius * -2.0f;
		XMVECTOR targetUp = XMVectorSet(0.0f, 1.0f, 0.0f, 0.0f);
		XMVECTOR targetPos = XMLoadFloat3(&m_SceneBounds.Center);

		XMMATRIX LightView = XMMatrixLookAtLH(lightPos, targetPos, targetUp);
		...
		XMMATRIX LightWorld = LightView * LightProj * T;

		XMStoreFloat4x4(&mLightView, LightView);
		XMStoreFloat4x4(&mLightProj, LightProj);
		XMStoreFloat4x4(&mShadowTransform, LightWorld);
	}
	...
	void D3DApp::OnKeyboardInput(const GameTimer* gameTimer) {
		...
		if ((GetAsyncKeyState(VK_UP) & 0x8000) != 0)
			m_SceneBounds.Center = { m_SceneBounds.Center.x, m_SceneBounds.Center.y, m_SceneBounds.Center.z + 1.0f };
		else if ((GetAsyncKeyState(VK_DOWN) & 0x8000) != 0)
			m_SceneBounds.Center = { m_SceneBounds.Center.x, m_SceneBounds.Center.y, m_SceneBounds.Center.z - 1.0f };
		else if ((GetAsyncKeyState(VK_RIGHT) & 0x8000) != 0)
			m_SceneBounds.Center = { m_SceneBounds.Center.x + 1.0f, m_SceneBounds.Center.y, m_SceneBounds.Center.z };
		else if ((GetAsyncKeyState(VK_LEFT) & 0x8000) != 0)
			m_SceneBounds.Center = { m_SceneBounds.Center.x - 1.0f, m_SceneBounds.Center.y, m_SceneBounds.Center.z };
		else if ((GetAsyncKeyState('O') & 0x8000) != 0)
			m_SceneBounds.Center = { m_SceneBounds.Center.x, m_SceneBounds.Center.y + 1.0f, m_SceneBounds.Center.z };
		...
		if ((GetAsyncKeyState('M') & 0x8000) != 0) {
			std::string str =
				"x: " +
				std::to_string(m_SceneBounds.Center.x) + ", " + 
				"y: " +
				std::to_string(m_SceneBounds.Center.y) + ", " +
				"z: " +
				std::to_string(m_SceneBounds.Center.z);

			MessageBoxA(0, str.c_str(), "Coordinate", MB_OK);
		}
	...
	void D3DApp::UpdatePassCB() {
		...
		XMMATRIX ST = XMLoadFloat4x4(&mShadowTransform);
		XMStoreFloat4x4(&m_MainPassCB.ShadowTransform, XMMatrixTranspose(ST));
	...
	void D3DApp::UpdateShadowPassCB() {
		XMMATRIX view = XMLoadFloat4x4(&mLightView);
		XMMATRIX proj = XMLoadFloat4x4(&mLightProj);

		XMMATRIX viewproj = XMMatrixMultiply(view, proj);

		XMFLOAT2 RTSize = XMFLOAT2((float)m_ShadowMap->GetWidth(), (float)m_ShadowMap->GetHeight());
		XMFLOAT2 InvRTSize = XMFLOAT2(1.0f / (float)m_ShadowMap->GetWidth(), 1.0f / (float)m_ShadowMap->GetHeight());
		...
		m_ShadowPassCB.RTSize = RTSize;
		m_ShadowPassCB.InvRTSize = InvRTSize;
		...
		auto currPassCB = m_CurrFrameResource->m_PassCB.get();
		currPassCB->CopyData(7, m_ShadowPassCB);
	}
	...
	void D3DApp::UpdateDynamicFaceCameraPassCBs() {
		for (int i = 0; i < 6; ++i) {
			...
			XMMATRIX ST = XMLoadFloat4x4(&mShadowTransform);
			XMStoreFloat4x4(&faceCameraPass.ShadowTransform, XMMatrixTranspose(ST));
	...
	void D3DApp::LoadTexture() {
		...
		iceTex->name = "ice";
		iceTex->filename = L"./Textures/snowcube1024.dds";
		THROWFAILEDIF("@@@ Error: DirectX::CreateDDSTextureFromFile12", DirectX::CreateDDSTextureFromFile12(m_Device.Get(), m_CommandList.Get(),
		...
		bricks2NormTex->filename = L"./Textures/bricks2_nmap.dds";
		THROWFAILEDIF("@@@ Error: DirectX::CreateDDSTextureFromFile12", DirectX::CreateDDSTextureFromFile12(m_Device.Get(), m_CommandList.Get(),
	...
	void D3DApp::BuildRenderItems() {
		...
		QuadRitem->World = VertexBuffer::GetMatrixIdentity4x4();
		QuadRitem->TexTransform = VertexBuffer::GetMatrixIdentity4x4();
		QuadRitem->InstanceCount = 14;
		QuadRitem->Mat = m_Materials["bricks0"].get();
		QuadRitem->Geo = m_DrawArgs["quadGeo"].get();
		...
		auto PlaneGridRitem3 = std::make_unique<RenderItem>();
		XMStoreFloat4x4(&PlaneGridRitem3->World, XMMatrixScaling(10.0f, 1.0f, 10.0f) * XMMatrixTranslation(0.0f, -30.0f, 0.0f));
		PlaneGridRitem3->TexTransform = VertexBuffer::GetMatrixIdentity4x4();
		PlaneGridRitem3->InstanceCount = 15;
		PlaneGridRitem3->Mat = m_Materials["planeWall2"].get();
		PlaneGridRitem3->Geo = m_DrawArgs["planeGeo"].get();
		PlaneGridRitem3->PrimitiveType = D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
		PlaneGridRitem3->IndexCount = PlaneGridRitem3->Geo->DrawArgs["grid"].IndexCount;
		PlaneGridRitem3->StartIndexLocation = PlaneGridRitem3->Geo->DrawArgs["grid"].StartIndexLocation;
		PlaneGridRitem3->BaseVertexLocation = PlaneGridRitem3->Geo->DrawArgs["grid"].BaseVertexLocation;
		PlaneGridRitem3->Visible = true;

		mRitemLayer[(int)RenderLayer::Opaque].push_back(PlaneGridRitem3.get());
		m_AllRItems.push_back(std::move(PlaneGridRitem3));
	}
	...
	void D3DApp::BuildDescriptorHeaps() {
		...
		for (size_t i = 0; i < names.size(); ++i) {
			auto Tex = m_Textures[names[i]]->GPUResource;
			srvDesc.Format = Tex->GetDesc().Format;
			srvDesc.Texture2D.MipLevels = Tex->GetDesc().MipLevels;
			m_Device->CreateShaderResourceView(Tex.Get(), &srvDesc, srvHandle);
		...
		m_ShadowMapIndex = m_SnowCubeMapTextureIndex + 1;
		mNullCubeSrvIndex = m_ShadowMapIndex + 1;
		mNullTexSrvIndex = mNullCubeSrvIndex + 1;

		srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURECUBE;
		srvDesc.TextureCube.MostDetailedMip = 0;
		srvDesc.TextureCube.MipLevels = iceTex->GetDesc().MipLevels;
		srvDesc.TextureCube.ResourceMinLODClamp = 0.0f;
		srvDesc.Format = iceTex->GetDesc().Format;

		auto cpuSrv = m_SrvDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
		auto gpuSrv = m_SrvDescriptorHeap->GetGPUDescriptorHandleForHeapStart();
		auto cpuDsv = m_DsvHeap->GetCPUDescriptorHandleForHeapStart();

		auto nullSrv = CD3DX12_CPU_DESCRIPTOR_HANDLE(cpuSrv, mNullCubeSrvIndex, m_CbvSize);
		m_NullSrv = CD3DX12_GPU_DESCRIPTOR_HANDLE(gpuSrv, mNullCubeSrvIndex, m_CbvSize);

		m_Device->CreateShaderResourceView(nullptr, &srvDesc, nullSrv);
		nullSrv.Offset(1, m_CbvSize);
		...
		m_ShadowMap->CreateDescriptor(
			CD3DX12_CPU_DESCRIPTOR_HANDLE(cpuSrv, m_ShadowMapIndex, m_CbvSize),
			CD3DX12_GPU_DESCRIPTOR_HANDLE(gpuSrv, m_ShadowMapIndex, m_CbvSize),
			CD3DX12_CPU_DESCRIPTOR_HANDLE(cpuDsv, 1, m_DsvSize));
	}
	...
	void D3DApp::BuildMaterials() {
		...
		planeW->MatCBIndex = 5;
		planeW->DiffuseSrvHeapIndex = 2;
		planeW->NormalSrvHeapIndex = 3;
		...
		planeW2->MatCBIndex = 6;
		planeW2->DiffuseSrvHeapIndex = 2;
	...
	void D3DApp::BuildQuadGeometry() {
		GeometryGenerator geoGen;
		GeometryGenerator::MeshData sphere = geoGen.CreateQuad(0.0f, 0.0f, 1.0f, 1.0f, 0.0f);
		...
		meshGeo->DrawArgs["quad"] = subMeshGeo;
		m_DrawArgs[meshGeo->Name] = std::move(meshGeo);
	}
	...
	void D3DApp::DrawSceneToShadowMap() {
		m_CommandList->RSSetScissorRects(1, &My_unmove(m_ShadowMap->Rect()));
		m_CommandList->RSSetViewports(1, &My_unmove(m_ShadowMap->Viewport()));
		...
		m_CommandList->ClearDepthStencilView(My_unmove(m_ShadowMap->Dsv()), D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, nullptr);
		m_CommandList->OMSetRenderTargets(0, nullptr, false, &My_unmove(m_ShadowMap->Dsv()));

		auto pass = m_CurrFrameResource->m_PassCB->Resource();
		D3D12_GPU_VIRTUAL_ADDRESS passGpuVirtualAddress = pass->GetGPUVirtualAddress() + (7 * passCstSize);
		m_CommandList->SetGraphicsRootConstantBufferView(1, passGpuVirtualAddress);
